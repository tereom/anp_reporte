...
Adicional a la comparación con otras Regiones CONANP comparamos los distintos indicadores dentro de las ANP y los equivalentes alrededor de las mismas, para esto se definieron los anillos como el área de los 25km circundantes a cada ANP.

Vale la pena notar que el anillo de las ANP puede no corresponder a la misma ecorregión, puede pertenecer a otra ANP cercana o puede representar un superficie mayor a la de la propia ANP, sin embargo, consideramos que, teniendo esto en cuenta, es informativo conocer los indicadores en estas zonas, las cuales representan el grado de presión antropogénica circundante. En el caso de la región `r mi_region` el área total de su anillo resulta en XXXXX hectáreas. La tabla del lado derecho indica la extensión de los anillos para las ANP totales en la region `r mi_region` en comparación con la extensión de los anillos y ANP en las otras regiones CONANP.
...
La tabla de la derecha nos muestra el porcentaje del área de la ANP 
*`r mi_region`* 
que pertence a cada clase de acuerdo al mapa 2010.

#### Tasa de transformación de cobertura boscosa
Veamos la pérdida decobertura forestal en la región año a año, y para cada tipo
de cobertura.
...
En las gráficas de abajo la línea roja representa la transformación de cobertura boscosa en la región `r mi_region`, 
la línea azul representa la misma transformación en el anillo circundante
y las líneas grises el equivalente en las otras regiones CONANP. En la 
gráfica del lado izquierdo podemos leer los resultados a total mientras 
