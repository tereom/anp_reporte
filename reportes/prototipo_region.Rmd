...
A lo largo del reporte agregamos comparativos con las regiones CONANP, la tabla de abajo indica cuantas hectáreas hay en las ANP correspondientes a cada región y el número de ANP de cada una.
...
mismas, para esto se definió a la periferia como el área de los 25 km circundantes a cada ANP que conforma la región. 

La tabla de la derecha indica las ANP de la región, las hectáreas que comprende
cada una y la extensión de la periferia que las rodean.
...
region_eco_df %>% 
    ...
    kable_styling(bootstrap_options = c("striped", "hover", "condensed"),
        position = "left", font_size = 11, full_width = FALSE) 
```
#### Periferia, Zonas núcleo y Zonas de Preservación

Adicional a la comparación con otras regiones CONANP comparamos los distintos indicadores dentro de las ANP los equivalentes alrededor de las mismas, para esto se definió a la periferia de 25 km circundante a cada ANP, así como en Zonas núcleo y Zonas de preservación si las ANP las presentan.

Vale la pena notar que la periferia de las ANP puede no corresponder a la misma ecorregión, puede pertenecer a otra ANP cercana o puede representar un superficie mayor a la de la propia ANP, sin embargo, consideramos que, teniendo esto en cuenta, es informativo conocer los indicadores en esta zona, la cual representa el grado de presión antropogénica circundante. La tabla del lado derecho indica la extensión de la periferia de las ANP totales en la region `r mi_region` en comparación con la extensión de la periferia y ANP en las otras regiones CONANP.
...
#### Tasa de transformación de hábitat
Veamos la pérdida de cobertura de vegetación en la región año a año, y para cada tipo
de cobertura.
...
En las gráficas de abajo la línea roja representa la transformación de hábitat en las ANP de la región `r mi_region`, 
la línea azul representa la misma transformación en la periferia circundante de las ANP
y las líneas grises la transformación en las ANP de las otras regiones CONANP. En la 
gráfica del lado izquierdo podemos leer los resultados, mientras 
...
pérdida de cobertura de vegetación media en hectáreas.
...
```
Adicionalmente, medimos la pérdida de vegetación como el porcentaje del área total de cada
...
pérdida que en el promedio de su ecorregión, el índice tomará un valor 
...
La integridad ecosistémica (IE) es un índice que relaciona de manera integral varios aspectos de la biodiversidad a través de modelos llamados redes bayesianas. Estos modelos representan relaciones intercruzadas entre variables descriptoras de los ecosistemas como lo son la estructura de la vegetación (altura de los árbooles, densidad de arbolado, el cobertura del dosel, diámetro del tallo, entre otras) mediante información obtenida del Inventario Nacional Forestal y Suelos (INFyS), producción primaria bruta, fotosíntesis neta estimada mediante imágenes satelitales (Landsat y MODIS) y variables contextuales como lo son el clima y la topografía. 
Dicho índice incorpora en el modelo los diferentes tipos de ecosistemas mediante las Zonas de Vida de Holdridge, de tal forma que considera la diversidad de estructura de vegetación que caracteriza cada ecosistema. Finalmente se relacionan dichas variables con la intensidad de cambios de uso de suelo que ha habido en el territorio nacional en el tiempo (INEGI). Con base en esto, los modelos arrojan un predicción sobre la condición actual de los ecosistemas del país. La IE fue elaborado por la CONABIO en colaboración con el INECOL A.C.en el año 2015.

Para este reporte se trabajó con el mapa de IE 2014, 
...
correspondiente a la periferia que la rodea.
...
Los indicadores faunísticos son complementarios con el índice de integridad y serán integrados al modelo bayesiano. Actualmente se evalúa la sensibilidad de estos indicadores a la degradación, y con el tiempo al integrar mayor cantidad de registros en el SNMB se podrán definir cuales son los mas permitentes.

### Depredadores tope de México, idoneidad de hábitat y presas principales

El Jaguar (*Panthera onca*) y el Puma (*Puma concolor*) son los depredadores tope en México por dos principales características ecológicas, sus hábitos estrictamente canívoros y su tamaño corporal. Ambos son los mas pesados entre los carnívoros estrictos en el país los cuales están entre un rango de peso de 30 kg a 158 kg (Burnie y Wilson 2001).

Para monitorear el estado de ambos depredadores se considera evaluar dos parámetros principales la tendencia de la calidad del  hábitat de cada especie de depredador a escala nacional regional y por ANP en las que se distribuyen y evaluar la prevalencia de sus presas principales mediante registros del Sistema Nacional de Monitoreo de la Biodiversidad (SNMB).

### Puma: Idoneidad de hábitat

La idoneidad de hábitat se refiere a las condiciones locales de calidad de hábitat del 
...
de la plataforma de Naturalista en vida silvestre.
...
los años 2008 a 2014. Los puntos azules corresponden a la idoneidad de hábitat en la periferia de las ANP 
de la región `r mi_region`.
...
los registros del *Puma concolor* del periodo completo (2008 al 2014 en este caso) 
ayudan a caracterizar el nicho ecológico con la mayor información disponible de la especie, 
es factible además reproyectar dichas condiciones al espacio geográfico anualmente (https://github.com/luismurao/hsi).
...
### Jaguar:Idoneidad de hábitat

(Pendiente modelos HSI para Jaguar)

# Presas principales del Puma y Jaguar

En México ha habido reportes sobre las presas más importantes, es decir aquellas que conforman más del 70 % de la biomasa consumida, entre los que se encuentran los venados (cola blanca y temazates), pecarí de collar y armadillo, algunas presas complementarias son el coatí, el tepezcuintle y los ceretes. En diversas localidades de Chamela-Cuixmala, Jalisco (Núñez *et al*, 2000) San Luis potosí (Rueda *et al*, 2013) y en Calakmul, Campeche (Estrada, 2008), las presas más importantes para el Jaguar son el pecarí de collar, los venados, el armadillo y el coatí. Otra presa un poco menos importantes fue el tepezcuintle. En todos los trabajos presentan que las presas más importantes para el Jaguar son el pecarí de collar, los venados, el armadillo y el coatí. Otra presa un poco menos importantes fue el tepezcuintle.

En cualquier parte, el espectro alimentario del puma o jaguar incluye entre 10 y 20 especies, pero las más importantes incluyen siempre ungulados (pecaríes y venados) y mamíferos medianos (coatí, armadillo y roedores). En ningún lugar las aves representan especies de importancia. Las especies más importantes tanto para el puma como para el jaguar son básicamente las mismas. Entre las especies que pueden considerarse exclusivas del puma estarían el borrego cimarrón y el venado bura; para el caso del jaguar, estaría el pecarí de labios blancos (en algunas localidades costeras los cocodrilos, las tortugas, los peces y las aves acuáticas pueden ser relevantes).

(Gráficas de Presas Principales y Abundancia Relativa por Regiones CONANP)
...
David Burnie y Don E. Wilson (2001). Animal: The Definitive Visual Guide to the World's Wildlife.
