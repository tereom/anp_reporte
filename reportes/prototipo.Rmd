...
El Área de Protección de Flora y Fauna Cuatrociénegas, está en el límite entre 
dos Provincias Geológicas, el Golfo de Sabinas y la Plataforma de Coahuila, 
donde la Sierra de La Fragua sirve como parte aguas.
...
mi_anp_shp_file <- list.files(path_anps_shp, 
    pattern = "negas.shp", recursive = FALSE) %>% # pattern = str_c(mi_anp, ".shp")
...
load("../datos_procesados/2017-10-07_ecorregion.RData")
load("../datos_procesados/2017-10-07_ecorregion_rings.RData")
...
parte de su terreno. En el caso de la ANP `r mi_anp` se le asignó la ecorregión
...
indicadores de las ANPs asignadas a la ecorregión `r str_c(mis_eco, collapse = ", ")`, 
...
En el caso de la ANP `r mi_anp` el área total de su anillo resulta en 
...
ecorregión que `r mi_anp`.


```{r}
load("../datos_procesados/2017-10-18_anp_region.RData")
anp_region_eco <- anp_region_cl %>% 
    left_join(anp_eco_df)
...
names(titulo_tab_region) <- c(mis_region)
anp_region_table %>% 
    kable("html", align = c("r", "c", "c"), padding = 10, 
        col.names = c("", "ha", "ha anillo")) %>%
    kable_styling(bootstrap_options = c("striped", "hover", "condensed"), 
        position = "float_right", font_size = 11, full_width = FALSE) %>% 
    row_spec(which(anp_region_table$anp == mi_anp), bold = F, color = "#79797d", background = "mistyrose") %>% 
...
anp_region_eco_table %>% 
    ...
    row_spec(which(anp_region_eco_table$region == mis_region), bold = F, color = "#79797d", background = "mistyrose") 
```
...
cobertura forestal entre 2010 y 2014.

```{r hansen_map, cache=TRUE}
hansen_loss <- raster("../datos_insumo/hansen_forest_loss_v1_3.tif")
...
```{r deforestacion_tiempo, cache=TRUE, fig.width = 8, fig.height=4.5}
load("../datos_procesados/2017-10-02_perdida_cobertura.RData")
load("../datos_procesados/2017-10-04_perdida_cobertura_rings.RData")
...
names(escala_color) <- c("otras", "anillo", mi_anp)
escala_alpha <- c(0.1, 0.8, 1)
names(escala_alpha) <- c("otras", "anillo", mi_anp)
...
perdida_anual_porcentaje_cl <- perdida_anual_porcentaje %>% 
    mutate(
        anp = ifelse(anp == stringr::str_c(mi_anp, "_ring"), 
            str_c(mi_anp, "_anillo"), anp),
        anp = stringr::str_replace_all(anp, "_", " ")
        ) %>% 
...
perdida_anual_cobertura_porcentaje <- perdida_anps_df %>% 
    ...
    mutate(percent_loss = round(n / sum(n) * 100, 3), 
        ha_loss = n * 3 * 3 / 100) %>%  
    ungroup() %>% 
    filter(year_loss > 1999, clase_madmex %in% c(1:3, 6)) %>% 
...
p2 <- ggplotly(perdida_anual_cobertura_plot, tooltip = vars_tooltip, 
                 dynamicTicks = TRUE)
subplot(style(p1, traces = 1:4, showlegend = FALSE), p2, margin = 0.03)
```
...
ANP perdida por deforestación en los últimos 5 años de datos, en este caso entre 2010 y 2014.
...
ggplot(indice_deforestacion_cl,
  ...
  labs(subtitle="Índice calculado por ecorregión", 
       title= "Pérdida boscosa en la región", x = "", y = "") + 
    coord_flip() +
...
ggplot(indice_deforestacion_cl, 
    aes(x = reorder(anp, ind_deforestacion), y = ind_deforestacion, label = eco, 
        group = anp)) + 
...
```{r mapa_ie, cache=TRUE, message=FALSE, warning=FALSE}
...
pal <- colorQuantile(palette = c("RdYlGn"), domain = NULL, n = 4,
    na.color = "transparent")
