...
params:
    mi_anp:
        value: anp_terrestres_2017_NOMBRE_Bonampak
---
...
mi_anp_ring_shp <- readOGR(path_anps_rings_shp, str_c(mi_anp_shp_file, "_ring"), 
    verbose = FALSE)
# mi_anp_ring_shp <- rmapshaper::ms_simplify(mi_anp_ring_shp)

# mi_anp_buffer_shp <- raster::union(mi_anp_shp, mi_anp_ring_shp)
```
...
mis_anps_df <- anp_eco_df %>% 
    filter(eco %in% mis_eco)
if(nrow(mis_anps_df) > 10) mis_anps_df <- filter(mis_anps_df, hectareas >= 1000)
mis_anps <- mis_anps_df %>% pull(anp) %>% union(mi_anp)
...
indicadores de otras ANPs de la misma ecorregión, esto se hace para poder 
...
madmex_mi_anp_c <-  case_when(
        ...
        madmex_mi_anp_v == 2 ~ "selva",
        madmex_mi_anp_v == 3 ~ "matorral",
        madmex_mi_anp_v == 4 ~ "pastizal",
        madmex_mi_anp_v == 5 ~ "sin vegetación aparente",
...
#         addTiles() %>%
        ...
        addRasterImage(madmex_mi_anp, color = pal, opacity = 0.6, 
            maxBytes = 8 * 1024 * 1024, project = FALSE) %>% 
...
y las líneas grises el equivalente en las ANPs de la ecorregión. En la 
gráfica del lado izquierdo podemos leer los resultados a total mientras 
...
perdida_anual_cobertura_porcentaje <- perdida_anps_df %>% 
    group_by(year_loss, anp, clase, clase_madmex) %>% 
    summarise(n = sum(n)) %>% 
...
tasa_cambio <- tasa_cambio_aux %>% 
    ...
    filter(year_loss > 2000) %>% 
    ungroup() %>% 
    select(anp, year_loss, tasa_cambio, clase)

tasa_cambio_cl <- tasa_cambio %>% 
  left_join(perdida_anual_porcentaje, by = c("anp", "year_loss", "clase")) %>% 
  left_join(anp_nombres, by = c("anp" = "anp_sin_acentos")) %>% 
    mutate(
        anp_corto = ifelse(anp == stringr::str_c(mi_anp, "_ring"), 
            str_c(mi_anp_corto, " anillo"), anp_corto),
        ) %>% 
    select(anp, ANP = anp_corto, año = year_loss, THH = tasa_cambio, perdida_ha = ha_loss, clase)

tasa_cambio_mi_anp <- filter(tasa_cambio_cl, anp == mi_anp)
tasa_cambio_mi_anp_ring <- filter(tasa_cambio_cl, anp == str_c(mi_anp, "_ring"))

tasa_cambio_plot <- ggplot(tasa_cambio_cl, aes(x = año, 
    y = THH, group = ANP, label = perdida_ha)) + 
    geom_line(aes(color = clase, alpha = clase)) +
    scale_alpha_manual(values = escala_alpha) +
    scale_color_manual(values = escala_color) +
    ...
    ylim(min(quantile(tasa_cambio_cl$THH, 0.2),
        min(tasa_cambio_mi_anp$THH),
        min(tasa_cambio_mi_anp_ring$THH)), 
        max(quantile(tasa_cambio_cl$THH, 0.8), 
            max(tasa_cambio_mi_anp$THH),
            max(tasa_cambio_mi_anp_ring$THH)))
...
tasa_cambio_clase <- tasa_cambio_clase_aux %>% 
    ...
    select(anp, year_loss, tasa_cambio, clase, clase_madmex)

tasa_cambio_clase_cl <- tasa_cambio_clase %>% 
  left_join(perdida_anual_cobertura_porcentaje, by = c("anp", "year_loss", "clase", "clase_madmex")) %>% 
  left_join(anp_nombres, by = c("anp" = "anp_sin_acentos")) %>% 
    mutate(
        anp_corto = ifelse(anp == stringr::str_c(mi_anp, "_ring"), 
            str_c(mi_anp_corto, " anillo"), anp_corto),
        ) %>% 
    select(anp, ANP = anp_corto, año = year_loss, THH = tasa_cambio, perdida_ha = ha_loss, 
           clase, clase_madmex)

tasa_cambio_clase_mi_anp <- filter(tasa_cambio_clase_cl, anp == mi_anp)
tasa_cambio_clase_mi_anp_ring <- filter(tasa_cambio_clase_cl, anp == str_c(mi_anp, "_ring"))
...
tasa_cambio_clase_plot <- ggplot(tasa_cambio_clase_cl, aes(x = año, 
    ...
    ylim(min(quantile(tasa_cambio_clase_cl$THH, 0.2),
        min(tasa_cambio_clase_mi_anp$THH),
        min(tasa_cambio_clase_mi_anp_ring$THH)), 
        max(quantile(tasa_cambio_clase_cl$THH, 0.8), 
            max(tasa_cambio_clase_mi_anp$THH),
            max(tasa_cambio_clase_mi_anp_ring$THH)))
...
positivo. La gráfica de abajo indica el valor de los índices para todas las ANPs
...
ANPs en la misma ecorregión y con los valores en los anillos.
...
de fototrampeo, huellas, excretas, registros fotográficos y grabadoras, del 
SNIB (Sistema Nacional de Información de la Biodiversidad) base de datos de 
CONABIO, y de la plataforma de Naturalista.
...
percepción remota (Land Sat). Así son asociadas al año correspondiente a los 
registros.

Vale la pena mencionar que se obtienen previamente coberturas ambientales 
anuales por lo que aunque todos los registros del periodo completo (2004 al 
2014 en este caso) ayudan por un lado a caracterizar el nicho ecológico con la 
mayor información disponible de la especie, por otro lado es así factible, 
reproyectar dichas condiciones al espacio geográfico anualmente. 

Dentro del modelo desarrollado se seleccionan las variables ambientales que 
...
ie_boxplot <- ggplot() +
    coord_flip() +
    geom_boxplot(data = tabla_suit, aes(x = reorder(anp_corto, suit, median),
        y = suit, color = clase), alpha = 0.6, show.legend = FALSE, 
        outlier.color = "gray90", coef = 0) +
    scale_color_manual("", values = escala_color) +
    geom_point(data = tabla_suit_rings_median, aes(x = anp_corto, 
