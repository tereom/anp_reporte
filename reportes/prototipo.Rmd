...
tasa_cambio <- tasa_cambio_aux %>% 
    ...
    filter(year_loss > 2000) %>% 
    ungroup() %>% 
    select(ANP = anp, año = year_loss, THH = tasa_cambio, clase)

tasa_cambio_mi_anp <- filter(tasa_cambio, ANP == mi_anp)
tasa_cambio_mi_anp_ring <- filter(tasa_cambio, ANP == str_c(mi_anp, "_ring"))

tasa_cambio_plot <- ggplot(tasa_cambio, aes(x = año, 
    y = THH, group = ANP)) + 
    geom_line(aes(color = clase, alpha = clase)) +
    scale_alpha_manual(values = escala_alpha) +
    scale_color_manual(values = escala_color) +
    labs(y = "%", x = "año", title = "THH (%)", color = "", 
         alpha = "") +
    ylim(min(quantile(tasa_cambio$THH, 0.2),
        min(tasa_cambio_mi_anp$THH),
        min(tasa_cambio_mi_anp_ring$THH)), max(tasa_cambio$THH))
...
tasa_cambio_clase <- tasa_cambio_clase_aux %>% 
    ...
    select(ANP = anp, año = year_loss, THH = tasa_cambio, clase, clase_madmex)

tasa_cambio_clase_mi_anp <- filter(tasa_cambio_clase, ANP == mi_anp)
tasa_cambio_clase_mi_anp_ring <- filter(tasa_cambio_clase, ANP == str_c(mi_anp, "_ring"))

tasa_cambio_clase_plot <-ggplot(tasa_cambio_clase, aes(x = año, 
    y = THH, group = ANP)) + 
    geom_line(aes(color = clase, alpha = clase)) +
    scale_alpha_manual(values = escala_alpha) +
    scale_color_manual(values = escala_color) +
    labs(y = "%", x = "año", title = "THH (%)", color = "", 
         alpha = "") +
    facet_wrap(~clase_madmex) +
    ylim(min(quantile(tasa_cambio_clase$THH, 0.2),
        min(tasa_cambio_clase_mi_anp$THH),
        min(tasa_cambio_clase_mi_anp_ring$THH)), max(tasa_cambio$THH))


vars_tooltip <- c("ANP", "tasa_cambio", "año")
...
pal <- colorBin(palette = c("Blues"), domain = c(1, 0), bins = 6,
...
leaflet(data = mi_anp_shp) %>% 
    ...
    fitBounds(lng1 = ext@xmin, lat1 = ext@ymin, lng2 = ext@xmax, lat2 = ext@ymax) %>% 
    addProviderTiles(providers$Esri.WorldImagery) %>% 
    addRasterImage(anp_buffer_suit, color = pal, opacity = 1) %>% 
...
leaflet(data = mi_anp_shp) %>% 
    ...
    fitBounds(lng1 = ext@xmin, lat1 = ext@ymin, lng2 = ext@xmax, lat2 = ext@ymax) %>% 
    addProviderTiles(providers$Esri.WorldImagery) %>% 
    addRasterImage(anp_buffer_75, color = "white", opacity = 0.8)
```
...
coords_sample <- sample(unique(tabla_suit_l$id), size = 1000)
