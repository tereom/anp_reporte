...
la línea azul representa la misma transformación en el anillo circunante
...
ext <- raster::extent(mi_anp_buffer_shp)
...
leaflet(data = mi_anp_shp) %>% 
    ...
    fitBounds(lng1 = ext@xmin, lat1 = ext@ymin, lng2 = ext@xmax, lat2 = ext@ymax) %>% 
    addProviderTiles(providers$Esri.WorldImagery) %>% 
    addRasterImage(anp_buffer_suit, color = pal, opacity = 0.8) %>% 
...
```{r}
p_75 <- quantile(values(anp_buffer_suit), prob = 0.75, na.rm = TRUE)
```

El mapa de abajo muestra las zonas de mayor idoneidad dentro de la ANP `r mi_anp_print`, 
los pixeles blancos corresponden al 25% del área de la ANP con mayor valor de 
idoneidad, en este caso la idoneidad en estas regiones es mayor a `r round(p_75, 2)`.

```{r}
anp_buffer_75 <- anp_buffer_suit > p_75
anp_buffer_75[anp_buffer_75 == 0] <- NA
...
leaflet(data = mi_anp_shp) %>% 
    ...
    fitBounds(lng1 = ext@xmin, lat1 = ext@ymin, lng2 = ext@xmax, lat2 = ext@ymax) %>% 
    addProviderTiles(providers$Esri.WorldImagery) %>% 
    addRasterImage(anp_buffer_75, color = "#8c2d04", opacity = 0.8)
```
