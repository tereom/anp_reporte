...
```{r madmex_map}
colores_madmex <- c("#4d642d", "#4cb974", "#e59f26", "#e0b762", "#9c9877", 
    "#b4b8bc", "#3f2f21", "#2eccfa")
...
leaflet() %>% 
    ...
    addLegend("bottomright", values = 1:10, colors = colores_madmex,
        title = "MAD-MEX (2015)", labels = c("bosque templado", "selva", 
            "matorral", "vegetación menor y pastizales", "tierras agrícolas", 
            "urbano y construido", "suelo desnudo", "agua"), opacity = 1) %>%
    addLayersControl(
        baseGroups = c("MAD-MEX", "Terreno"),
        overlayGroups = c("ANP", "GAP", "Loc 2500", "Periferia", "Núcleo-Pres."),
...
```{r ie_map, eval=TRUE}
colores_ie <- c("#ff0000", "#e82927", "#f98939", "#fec981", "#ffedab", 
    "#ecf7ad", "#c4e687", "#97d265", "#50a14a", "#0c461d")
etiquetas_ie <- c("0.0 - 0.1", "0.1 - 0.2", "0.2 - 0.3", "0.3 - 0.4", 
...
leaflet() %>% 
    ...
    addLegend("bottomright", values = 1:10, colors = colores_ie,
        title = "IE", labels = etiquetas_ie, opacity = 1) %>%
    addLayersControl(
        baseGroups = c("Integridad", "Terreno"),
        overlayGroups = c("ANP", "GAP", "Loc 2500", "Periferia", "Núcleo-Pres."),
        options = layersControlOptions(collapsed = FALSE)
        )  %>%
    hideGroup("GAP") %>% 
...
```{r ih_boxplot, fig.width = 4.5, fig.height=6, include=FALSE, eval=TRUE}
load("../datos_procesados/2018-02-12_suitability.RData")
...
```{r ih_boxplot_2, fig.width = 4.5, fig.height=6, eval=TRUE}
...
leaflet() %>%
    ...
    addLayersControl(
        baseGroups = c("IH Jaguar", "Terreno"),
        overlayGroups = c("ANP", "GAP", "Loc 2500", "Periferia", "Núcleo-Pres."),
        options = layersControlOptions(collapsed = FALSE)
        )  %>%
    hideGroup("GAP") %>% 
...
```{r, fig.width=4, fig.height=3, echo=FALSE, eval=FALSE, error=TRUE}
...
```{r ih_boxplot_ponca, fig.width = 4.5, fig.height=6, include=FALSE, eval=TRUE}
...
```{r ih_boxplot_2_ponca, fig.width = 4.5, fig.height=6, eval=TRUE}
ih_stats_2014 <- suit_stats %>% 
    ungroup() %>% 
    filter(anio == 2014, tipo != "ANP", anp %in% mis_anps_region) %>% 
    select(anp_sin_acentos = anp, mediana, tipo) %>% 
